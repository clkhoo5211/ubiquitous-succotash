...
from src.schemas.like import LikeResponse, LikeListResponse
...
@router.delete(
    "/posts/{post_id}/like", status_code=status.HTTP_204_NO_CONTENT, summary="Unlike a post"
)
...
@router.get(
    "/posts/{post_id}/likes", response_model=LikeListResponse, summary="Get users who liked a post"
)
async def get_post_likes(
    ...
    **Returns:**
    ...
    return LikeListResponse(
        likes=likes, total=total, page=page, page_size=page_size, total_pages=total_pages
    )
...
async def get_user_likes(
    ...
    content_type: Optional[str] = Query(
        None, description="Filter by content type: 'post' or 'comment'"
    ),
    db: AsyncSession = Depends(get_db),
):
    """
    ...
    **Returns:**
    ...
    likes, total = await like_service.get_user_likes(
        user_id=user_id, page=page, page_size=page_size, content_type=content_type
    )
