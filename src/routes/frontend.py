...
async def explore(request: Request, filter: Optional[str] = None, page: int = 1):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if filter == "hot":
            ...
            # Sort by most comments
            posts = sorted(posts, key=lambda p: p.comment_count if p.comment_count else 0, reverse=True)
...
async def leaderboard(request: Request):
    ...
    async with AsyncSessionLocal() as db:
        ...
        result = await db.execute(
            ...
            .group_by(User.id)
            .having(func.coalesce(func.count(Post.id), 0) + func.coalesce(func.count(Comment.id), 0) > 0)
            .order_by(desc(func.coalesce(func.count(Post.id), 0) + func.coalesce(func.count(Comment.id), 0)))
            .limit(50)
        )
...
async def register(request: Request):
    ...
    return templates.TemplateResponse(
        "auth/register.html",
        await get_template_context(request)
    )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 201:
                ...
                redirect = RedirectResponse(url="/?success=Registration+successful!+Welcome+to+the+forum.", status_code=303)
        ...
        # Error - redirect back
        return RedirectResponse(
            url=f"/auth/register?error=Registration+failed",
            status_code=303
        )
...
async def login_post(
    ...
    try:
        ...
        # Error - redirect back
        return RedirectResponse(
            url=f"/auth/login?error=Login+failed",
            status_code=303
        )
...
async def profile(request: Request, username: str):
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            # Get user's posts
            post_service = PostService(db)
            posts, total_posts = await post_service.list_posts(author_id=profile_user.id, page=1, page_size=20)
...
async def create_post_post(
    ...
    try:
        ...
        if not current_user:
            return RedirectResponse(
                url=f"/auth/login?error=Not+authenticated",
                status_code=303
            )
        ...
        traceback.print_exc()
        return RedirectResponse(
            url=f"/posts/create?error=Post+creation+failed: {str(e)}",
            status_code=303
        )
...
async def create_comment_post(
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            return RedirectResponse(
                url=f"/posts/{post_id}#comment-{new_comment.id}",
                status_code=303
            )
    except Exception as e:
        # Error - redirect back
        ...
        traceback.print_exc()
        return RedirectResponse(
            url=f"/posts/{post_id}?error=Comment+failed: {str(e)}",
            status_code=303
        )
...
async def like_post_handler(
    ...
    try:
        ...
        traceback.print_exc()
        return RedirectResponse(
            url=f"/posts/{post_id}?error=Like+failed: {str(e)}",
            status_code=303
        )
...
async def settings_page(request: Request):
    ...
    if not context.get("current_user"):
        return RedirectResponse(
            url="/auth/login?error=Please+login+to+access+settings",
            status_code=303
        )
...
async def update_profile(
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/settings?error=Failed+to+update+profile: {str(e)}",
            status_code=303
        )
...
async def change_password(
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/settings?error=Failed+to+change+password: {str(e)}",
            status_code=303
        )
...
async def update_wallet(
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            return RedirectResponse(
                url="/settings?success=Wallet+address+updated+successfully",
                status_code=303
            )
    except Exception as e:
        return RedirectResponse(
            url=f"/settings?error=Failed+to+update+wallet: {str(e)}",
            status_code=303
        )
...
async def update_notifications(
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            return RedirectResponse(
                url="/settings?success=Notification+settings+updated+successfully",
                status_code=303
            )
    except Exception as e:
        return RedirectResponse(
            url=f"/settings?error=Failed+to+update+notifications: {str(e)}",
            status_code=303
        )
...
async def delete_account(request: Request):
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/settings?error=Failed+to+delete+account: {str(e)}",
            status_code=303
        )
